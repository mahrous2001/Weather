...
async function api(countryName) {
  let x = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=5d00b4d2a3604150927120437240201&q=${countryName}&days=3`
  );
  let y = await x.json();
  displayWeather(y);
}
  ...
  cartona = `<div class="col-lg-4">
                    <div class="bg-content rounded-4 text-white">
                        ...
                        <div class="d-flex justify-content-between p-3">
                            <h4>${
                              a?.current?.temp_c
                            }<span class="topp">o</span>C</h4>
                            <img class="position-relative left" src="https:${
